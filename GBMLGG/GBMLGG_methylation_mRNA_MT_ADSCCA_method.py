...
##############################
##############################
kidtx = pd.read_csv('GBMLGG__methylation_mRNA_raw_data.csv')
dataX01 = kidtx.drop(["Unnamed: 0","Hybridization_REF","t_e2...1.","t_e2...2.", "X"], axis = 1)
dataX1 = kidtx.drop(["Unnamed: 0","Hybridization_REF","t_e2...1.","t_e2...2.", "X"], axis = 1).values

y = np.transpose(np.array((kidtx["t_e2...1."], kidtx["t_e2...2."]))) # V1=time; erged_data33=status

[ m0,n0] = dataX1.shape

col_name=list(dataX01)
y_name=["t_e2...1.","t_e2...2."]
...
ytime=np.transpose(np.array(kidtx["t_e2...1."])) # only V1=time;
ystatus= np.transpose(np.array(kidtx["t_e2...2."])) #only erged_data33=status
...
#    plt.legend(['${lambda0}$','${lambda1}$'], loc='upper right')
    ...
    np.savetxt("GBMLGG_methylation_mRNA_MT-ADSCCA_20210902.csv",hist.history['loss'], delimiter=",")
...
#np.savetxt("GBMLGG_data_mm_dscca_p001_8-9_20210910.csv",data_mm2, delimiter=",")

#pd.dataFrame.to_save()
data_y_mm_name55.to_csv("GBMLGG_data_mm_dscca.csv")
